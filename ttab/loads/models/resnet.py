...
class ResNetAffectNet(ResNetBase):
    def __init__(
        self,
        num_classes: int,
        depth: int = 50,  # typically 50, 101, or 152 for SOTA performance
        split_point: str = "layer4",
        group_norm_num_groups: int = None,
        grad_checkpoint: bool = False,
    ):
        super(ResNetAffectNet, self).__init__()
        self.num_classes = num_classes
        if split_point not in ["layer3", "layer4", None]:
            raise ValueError(f"invalid split position={split_point}.")
        self.split_point = split_point
        ...
        # Select block type and layer configuration based on depth.
        if depth in [50, 101, 152]:
            block_fn = Bottleneck
            layers_config = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3]}[depth]
        elif depth in [18, 34]:
            block_fn = BasicBlock
            layers_config = {18: [2, 2, 2, 2], 34: [3, 4, 6, 3]}[depth]
        else:
            raise ValueError("Unsupported depth for ResNetAffectNet: choose 18, 34, 50, 101, or 152.")

        self.block_fn = block_fn
        self.layers_config = layers_config

        # For AffectNet, we use the standard ImageNet stem.
        self.inplanes = 64
        self.conv1 = nn.Conv2d(
            in_channels=3,
            out_channels=64,
        ...
        self.layer1 = self._make_layer(
            block_fn, planes=64, blocks=layers_config[0], group_norm_num_groups=group_norm_num_groups
        )
        self.layer2 = self._make_layer(
            block_fn, planes=128, blocks=layers_config[1], stride=2, group_norm_num_groups=group_norm_num_groups
        )
        self.layer3 = self._make_layer(
            block_fn, planes=256, blocks=layers_config[2], stride=2, group_norm_num_groups=group_norm_num_groups
        )
        self.layer4 = self._make_layer(
            block_fn, planes=512, blocks=layers_config[3], stride=2, group_norm_num_groups=group_norm_num_groups
        )

        # Global average pooling and classifier.
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.classifier = nn.Linear(512 * block_fn.expansion, self.num_classes, bias=False)
    ...
    def forward_features(self, x):
        """Forward pass up to the global pooling. Uses gradient checkpointing if enabled."""
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
    ...
    def forward_head(self, x, pre_logits: bool = False):
        """Forward pass for the remaining block (if split earlier) and the classifier."""
        if self.split_point == "layer3":
            if self.grad_checkpoint:
                x = checkpoint_seq(self.layer4, x, preserve_rng_state=True)
            else:
    ...
    def _make_layer(self, block_fn, planes, blocks, stride=1, group_norm_num_groups=None):
        """Helper function to create one ResNet stage."""
        downsample = None
        if stride != 1 or self.inplanes != planes * block_fn.expansion:
            downsample = nn.Sequential(
        ...
        layers = []
        layers.append(block_fn(self.inplanes, planes, stride, downsample, group_norm_num_groups))
        self.inplanes = planes * block_fn.expansion
